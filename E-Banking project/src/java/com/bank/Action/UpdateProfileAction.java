	 ...
	 try {
		...
		try{
        	...
        	String login=(String)session.getAttribute("loginuser");
        	   ...
        	   flag=new RegisterDaoImpl().updateProfile(rto);
			...
			if(flag){
				...
				request.setAttribute("status", "Updation of u r profile is successfull");
				}
			else{
				
               path="./userhome.jsp";
				
				request.setAttribute("status", "Updation of u r profile is Failed");
				}
			}catch (Exception e) {
			e.printStackTrace();
